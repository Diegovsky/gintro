...
fn Iterator(comptime Iterable: type, comptime Get: type, comptime index: fn(*Iterable, i32)*Get.Raw, comptime len: fn(*Iterable)i32) type {
    return struct {
        iterable: *Iterable,
        len: i32,
        i: i32,
        ...
        pub fn new(iterable: *Iterable) @This() {
            return .{.iterable = iterable, .len = len(iterable), .i = 0};
        }
        ...
        pub fn next(self: *@This()) ?Get {
            ...
            defer self.i += 1;
            return Get.fromC(index(self.iterable, self.i));
        }
    };
}
...
pub const Repository = struct {
    ...
    /// Just a wrapper for the Iterator pattern glib uses.
    /// This is because the [Iterator] type assumes 1 parameter. This uses 2.
    const InfoIteratorContext = struct {
        ...
        pub fn get(self: *@This(), i: i32) *Raw {
            return C.g_irepository_get_info(self.repo, self.namespace.name, i);
        }
    };
    const InfoIterator = Iterator(InfoIteratorContext, BaseInfo, InfoIteratorContext.get, InfoIteratorContext.len);

    pub fn getInfoIterator(self: *Self, namespace: *Namespace) InfoIterator {
        return InfoIterator.new(InfoIteratorContext{.namespace=namespace, .repo=self.raw});
    }
};
...
pub const BaseInfo = struct {
    ...
    pub fn downcast(self: *const Self, comptime T: type) ?T  {
        if (T.is(self)) {
...
pub const StructInfo = struct {
    ...
    const FieldsIterator = Iterator(Self.Raw, FieldInfo, C.g_struct_info_get_field, C.g_struct_info_get_n_fields);

    pub fn getFieldsIterator(self: Self) FieldsIterator {
        return FieldsIterator.new(self.raw);
    }
